...
color1 = cmyk_to_hex(*color1_cmyk)
color2 = cmyk_to_hex(*color2_cmyk)
# color_background = cmyk_to_hex(*color_background_cmyk)
color_background = '#c81f32'
...
d = drawSvg.Drawing(2.1, 2.1, origin='center', fill='red')
d.draw(euclid.shapes.Circle(0, 0, 1), stroke='none', fill=color_background)

# d.draw(drawSvg.Rectangle(-1,-1,2,2, fill=color_background))

N = nb_geodesics + 1
angles = [horo_angle + 2*math.pi*i/N for i in range(1,N)]
colors = color_gradient(nb_geodesics, *color1_cmyk)
...
colors = color_gradient(nb_horocycles, *color2_cmyk)
for i in range(nb_horocycles):
    draw_horocycle(horoList[i], color=colors[i])
...
d.draw(euclid.shapes.Circle(0, 0, 1), stroke='#4f0c12', stroke_width=2*stroke_width, fill='none')
